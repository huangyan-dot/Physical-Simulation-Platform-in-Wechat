...
阶段 0 骨架 + 阶段 1 全量接线 + M2 中间件：Gin + GORM + MySQL + viper + zap，已验证全链路（HTTP -> 路由 -> middleware -> controller -> service -> repository -> MySQL -> JSON）。
...
1. 接 Apifox 跑一遍 13 接口联调（dev 后门即可全链路验证）
