...
//   - 3 个实验（config + target，含 pass_score）
//   - 3 个关卡（串成解锁链 L1→L2→L3）
...
func seedExperiments(db *gorm.DB, logger *zap.Logger) {
	...
	seeds := []expSeed{
		...
		{
			// angle_deg 默认 5°：与原理页「摆角应小于 5°」的要求一致（此前是 15°，自相矛盾）。
			// gravity 只作为未上报定位时的兜底真值，正常评分用学生所在县级地区的当地 g。
			code: "pendulum", name: "单摆实验", renderMode: "mixed_3d_2d",
			config: `{"length_m":1.0,"angle_deg":5,"gravity":9.8,"local_gravity":true}`,
	...
	for _, s := range seeds {
		...
		} else if s.code == "pendulum" {
			// 老库回填：早期种子把 angle_deg 写成 15°，与原理页要求的 <5° 冲突；
			// 且缺 local_gravity 开关。只在检测到旧值时覆盖，不动教师自定义的其他字段。
			var cfg map[string]interface{}
			if json.Unmarshal([]byte(e.Config), &cfg) == nil {
				changed := false
				if v, ok := cfg["angle_deg"].(float64); ok && v > 5 {
					cfg["angle_deg"] = 5
					changed = true
				}
				if _, ok := cfg["local_gravity"]; !ok {
					cfg["local_gravity"] = true
					changed = true
				}
				if changed {
					if b, mErr := json.Marshal(cfg); mErr == nil {
						if uErr := db.Model(&e).Update("config", model.JSON(b)).Error; uErr != nil {
							logger.Warn("回填单摆配置失败", zap.Error(uErr))
						} else {
							logger.Info("已回填单摆实验配置", zap.String("config", string(b)))
						}
					}
				}
			}
...
// seedLevels 幂等写入 3 个关卡并串成解锁链。
// 已有任一关卡则跳过，避免破坏既有 prereq 链。
func seedLevels(db *gorm.DB, logger *zap.Logger) {
	var count int64
	if err := db.Model(&model.Level{}).Count(&count).Error; err != nil {
		logger.Warn("seed 关卡计数失败", zap.Error(err))
		return
	}
	if count > 0 {
		return
	...
	// 顺序与 difficulty 与前端 utils/mock.js 一致
	seeds := []lvSeed{
...
func seedDemoClass(db *gorm.DB, logger *zap.Logger) {
	...
	if err == gorm.ErrRecordNotFound {
		cls = model.Class{Name: className, TeacherID: teacher.ID, Code: "PHYS1B"}
	...
	// 补全旧数据的班级码（AutoMigrate 加字段后旧行 code 为空）
	if cls.Code == "" {
		db.Model(&cls).Update("code", "PHYS1B")
	}
}
