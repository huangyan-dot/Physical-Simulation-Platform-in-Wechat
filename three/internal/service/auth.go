...
const (
	maxNameLen      = 64 // DB VARCHAR(64)
	maxStudentNoLen = 32 // DB VARCHAR(32)
)
...
func NewAuthService(userRepo *repository.UserRepository, classRepo *repository.ClassRepository, logRepo *repository.OperationLogRepository, jm *jwt.Manager, wx *wechat.Client, allowDevBackdoor bool) *AuthService {
	return &AuthService{
		userRepo: userRepo, classRepo: classRepo, logRepo: logRepo, jm: jm, wx: wx,
...
func (s *AuthService) Login(code string) (token string, user *LoginUser, err error) {
	...
	// 审计日志：记录登录（M6 验收要求"登录有日志"）
	if s.logRepo != nil {
		_ = s.logRepo.Create(&model.OperationLog{
			UserID: u.ID,
			Action: "login",
			Detail: fmt.Sprintf("openid=%s role=%s", u.OpenID, u.Role),
		})
	}
...
func (s *AuthService) CompleteProfile(userID int64, name, studentNo string, classID *int64) (*model.User, error) {
	...
	if studentNo == "" || len(studentNo) > maxStudentNoLen {
		return nil, ErrBadRequest
	}
