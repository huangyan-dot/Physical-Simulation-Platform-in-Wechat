...
// ListExperiments GET /experiments（契约 §17）
func (ctl *LevelController) ListExperiments(c *gin.Context) {
	rows, err := ctl.svc.ListExperiments()
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"code": 500, "message": "加载实验列表失败"})
		return
	}
	if rows == nil {
		rows = []model.ExperimentSummary{}
	}
