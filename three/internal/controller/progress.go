...
func (ctl *ProgressController) Mine(c *gin.Context) {
	view, err := ctl.svc.Mine(middleware.CurrentUserID(c))
	if err != nil {
...
func (ctl *ProgressController) Submit(c *gin.Context) {
	var req struct {
		...
		UserConfig json.RawMessage `json:"user_config"` // v0.5：可选，前端实际实验参数（滑块问题修复）
	}
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"code": 400, "message": "参数错误：level_id/experiment 必填"})
		return
	}
	res, err := ctl.svc.Submit(middleware.CurrentUserID(c), req.LevelID, req.Experiment, req.Readings, req.UserConfig)
	if err != nil {
		if !writeServiceErr(c, err) {
