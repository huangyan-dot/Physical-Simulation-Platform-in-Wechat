...
func seedExperiments(db *gorm.DB, logger *zap.Logger) {
	...
	seeds := []expSeed{
		...
		{
			// 光电效应：h=6.626e-34 J·s, W=3.0e-19 J (约 1.87 eV, 钾)
			code: "photoelectric", name: "光电效应实验", renderMode: "mixed_3d_2d",
			config: `{"wavelengths_nm":[405,436,546,577]}`,
			target: `{"method":"planck_fit","planck_const":6.626e-34,"work_function":3.0e-19,"pass_score":60}`,
		},
		{
			// 弗兰克-赫兹：汞原子激发电位 4.9V
			code: "frank_hertz", name: "弗兰克-赫兹实验", renderMode: "mixed_3d_2d",
			config: `{"vg2_min":0,"vg2_max":30,"step":0.2}`,
			target: `{"method":"excitation_fit","excitation_pot_v":4.9,"pass_score":60}`,
		},
		{
			// 光栅衍射：每毫米 300 条 -> d = 1/300 mm ≈ 3.333e-6 m，钠光 589.3nm
			code: "diffraction_grating", name: "光栅衍射实验", renderMode: "mixed_3d_2d",
			config: `{"grating_const_m":3.333e-6,"wavelength_nm":589.3}`,
			target: `{"method":"grating_fit","grating_const_m":3.333e-6,"pass_score":60}`,
		},
		{
			// 密立根油滴：元电荷 e = 1.602e-19 C
			code: "oil_drop", name: "密立根油滴实验", renderMode: "mixed_3d_2d",
			config: `{"plate_dist_m":0.006,"oil_density":850,"air_viscosity":1.83e-5}`,
			target: `{"method":"oil_drop_fit","elementary":1.602e-19,"pass_score":60}`,
		},
		{
			// 偏振光：验证马吕斯定律 I = I₀cos²(θ)
			code: "polarization", name: "偏振光实验", renderMode: "mixed_3d_2d",
			config: `{"initial_intensity":1.0}`,
			target: `{"method":"malus_fit","pass_score":60}`,
		},
		{
			// 声速测量：v = λ·f，f=37kHz，室温 v≈343 m/s
			code: "sound_speed", name: "声速测量实验", renderMode: "mixed_3d_2d",
			config: `{"freq_hz":37000}`,
...
func seedLevels(db *gorm.DB, logger *zap.Logger) {
	...
	seeds := []lvSeed{
		...
		{"photoelectric", "光电效应实验", 7, 3},
		{"frank_hertz", "弗兰克-赫兹实验", 8, 5},
		{"diffraction_grating", "光栅衍射实验", 9, 3},
		{"oil_drop", "密立根油滴实验", 10, 4},
		{"polarization", "偏振光实验", 11, 2},
		{"sound_speed", "声速测量实验", 12, 3},
	}
