...
func main() {
	...
	if err := db.AutoMigrate(
		...
		&model.Assignment{}, &model.AssignmentSubmission{},
	); err != nil {
	...
	asgRepo := repository.NewAssignmentRepository(db)

	authSvc := service.NewAuthService(userRepo, classRepo, jm, wx, cfg.AllowDevBackdoor, cfg.TeacherInviteCode)
	...
	asgSvc := service.NewAssignmentService(asgRepo, classRepo, levelRepo, expRepo, progRepo, logRepo, userRepo)
	...
	deps := router.Deps{
		...
		AssignmentCtl:    controller.NewAssignmentController(asgSvc),
