...
func (r *ClassRepository) ListByUser(userID int64, role string) ([]model.ClassView, error) {
	q := r.db.Table("classes c").
		Select("c.id, c.name, c.code, c.teacher_id, u.name AS teacher_name, COUNT(cm.id) AS member_count").
...
// FindByCode 按班级码查班级（学生自助加入用）
func (r *ClassRepository) FindByCode(code string) (*model.Class, error) {
	var c model.Class
...
// ListMembers 班级成员明细（教师查名单用）
func (r *ClassRepository) ListMembers(classID int64) ([]MemberView, error) {
	var rows []MemberView
	err := r.db.Table("class_members cm").
		Select("cm.user_id, u.name, u.student_no, cm.joined_at").
